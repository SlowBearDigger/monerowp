/*! <monero-pay> checkout widget. Self-hosted; no CDN or third-party requests. */
(function(){
// Uses the locally bundled qrcode dependency. Network calls are limited to
// merchant endpoints configured by the plugin.
...
// sans). skin="brutal" flips to mono/square/hard-offset shadow. every
// color/shape is a css custom property so any brand can retheme without forking.
var XP_CSS = [
    ...
    '.hidden{display:none;}',
    '@media (prefers-reduced-motion:reduce){.tgl .car,.xp-steps .s,.xp-steps .s .bar{transition:none;}}',
].join('');
...
// Stack…) prefills the same clean amount (wallet2-compatible form). odd inputs
// are left untouched (the amount is validated server-side regardless).
...
// signed-config verification, browser side. Ed25519 via WebCrypto (modern browsers).
// Canonical JSON must stay byte-stable: sorted keys, no whitespace.
...
async function xpFingerprint(der) {
    // 12 bytes / 96 bits of SHA-256 — short id shown in the trust panel.
...
class MoneroPay extends HTMLElement {
    static get observedAttributes() { return ['address', 'amount', 'label', 'order', 'verify-url', 'status-url', 'stream-url', 'lang', 'redirect-url', 'theme', 'show-qr']; }
    ...
    _closeStream() { if (this._es) { try { this._es.close(); } catch (e) { /* already closed */ } this._es = null; } this._streaming = false; }
    ...
    _render() {
        ...
        if (!/^[1-9A-HJ-NP-Za-km-z]{95}$/.test(addr)) {
            root.innerHTML = '<style>' + XP_CSS + '</style><div class="card"><div class="hd"><div class="lbl">Monero</div>' +
        ...
        root.innerHTML =
            ...
            '</div>' +
            '<div class="body">' +
            (showQr ? '<div class="qrwrap"><div class="qr">' + xpQrSvg(this._uri()) + '</div></div>' : '') +
            '<div class="sec">' +
            ...
            '</div>' +
            // watch mode: the scanner is already scanning (view key), so the buyer
    ...
    _wire(root, addr, verifyUrl, t) {
        ...
        if (addrBtn) addrBtn.addEventListener('click', function () {
            var done = function () {
                ...
                var oldLabel = addrBtn.getAttribute('aria-label');
                addrBtn.textContent = t.copied;
                addrBtn.setAttribute('aria-label', t.copied);
                setTimeout(function () {
                    addrBtn.innerHTML = old;
                    if (oldLabel) addrBtn.setAttribute('aria-label', oldLabel);
                }, 1600);
            };
    ...
    async _verify(root, verifyUrl, t) {
        ...
        this.dispatchEvent(new CustomEvent('monero-pay:result', { detail: out, bubbles: true, composed: true }));
    ...
    // push stream: connect to the scanner's SSE channel (stream-url). each event is a
    ...
    // watch mode poll: GET the status URL (the scanner /order/:id or a proxy) and
    // react. no proof to paste — the scanner is already scanning the chain. keeps
    ...
    _success(root, out, t) {
        ...
        this.dispatchEvent(new CustomEvent('monero-pay:paid', { detail: out, bubbles: true, composed: true }));
    ...
    // fetch the signed receipt (from receipt-url, the scanner /receipt/:id or a
    ...
    async _receipt(root, t) {
        ...
        if (!url || !box) return;
        // the scanner may still be minting the receipt (the on-chain tx_proof can
        ...
        // UTF-8 → base64 without deprecated unescape().
        var b64 = btoa(encodeURIComponent(JSON.stringify(env)).replace(/%([0-9A-F]{2})/gi, function (_, p1) {
