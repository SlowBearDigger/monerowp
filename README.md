...
WooCommerce payment gateway for Monero (XMR). The merchant sets a primary address, private view key, and one or more monerod nodes. WordPress derives a unique subaddress per order and scans the blockchain in PHP to detect and confirm payment. No `monero-wallet-rpc`. No third-party block explorer. The private view key never leaves the server.

**Contributors:** serhack, mosu-forge and Monero Integrations contributors<br>
**Donate:** https://monerointegrations.com/donate.html<br>
**License:** [MIT](LICENSE)<br>
**Repository:** https://github.com/monero-integrations/monerowp

## Features

* Watch-mode verification against monerod daemon RPC (HTTP/HTTPS), with ordered node failover and optional Basic or Digest authentication
...
* Checkpointed, bounded scanning; five-minute reconciliation and hourly expiry checks through cron
...
* WordPress 6.2+, PHP 8.0+, WooCommerce 7.0+
...
* monerod daemon RPC reachable from the WordPress host and on the same network as the address
...
1. Download from the [releases page](https://github.com/monero-integrations/monerowp/releases) or `git clone https://github.com/monero-integrations/monerowp`
...
| Network | Detected from the primary address |
...
| Monero node(s) | Ordered HTTP(S) nodes with optional Basic or Digest authentication; all must match the address network |
| Confirmations required | Default 1; `0` = mempool |
| Underpayment tolerance (XMR) | Accept a configured shortfall from 0 up to, but not including, 0.01 XMR |
